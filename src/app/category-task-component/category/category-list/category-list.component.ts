...
export class CategoryListComponent {
   ...
   * 
   * @param category - this is the category that was selected from the menu.
   */
  public onSelectCategory(category: Menu): void {
   ...
   * 
   * @param initialSelectedCategory - this is a boolean value that is used to determine whether
   * or not to set the selected category to the last category in the list. This is used when
   * the user is creating a new task.
   */
  private getCategories(initialSelectedCategory: boolean) {
   ...
   */
  public addNewCategoryMenu(categoryName: string): void {
   ...
   * @return The number of times a category name appears in the menu.
   */
  private countExistingCategory(name: String) {
    var count = 0;
   ...
   * 
   * @param category - this is the parameter that we are passing in from the menu.ts file.
   */
  private onSelected(category: Menu) {
    this.taskService.setSelectedCategory(category);
  }
